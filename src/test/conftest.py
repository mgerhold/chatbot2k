import os

os.environ.setdefault("ENVIRONMENT", "production")
os.environ.setdefault("DATABASE_FILE", "dummy")
os.environ.setdefault("DATA_ROOT_PATH", "./data")
os.environ.setdefault("TWITCH_CLIENT_ID", "dummy")
os.environ.setdefault("TWITCH_CLIENT_SECRET", "dummy")
os.environ.setdefault("TWITCH_ACCESS_TOKEN", "dummy")
os.environ.setdefault("TWITCH_REFRESH_TOKEN", "dummy")
os.environ.setdefault("TWITCH_CHANNEL", "dummy")
os.environ.setdefault("TWITCH_CHATBOT_WEB_INTERFACE_CLIENT_ID", "dummy")
os.environ.setdefault("TWITCH_CHATBOT_WEB_INTERFACE_CLIENT_SECRET", "dummy")
os.environ.setdefault("TWITCH_REDIRECT_URI", "dummy")
os.environ.setdefault("TWITCH_EVENTSUB_PUBLIC_URL", "dummy")
os.environ.setdefault("TWITCH_EVENTSUB_LISTEN_PORT", "8081")
os.environ.setdefault("TWITCH_EVENTSUB_SECRET", "dummy")
os.environ.setdefault("IGNORE_EXISTING_SUBSCRIPTIONS", "false")
os.environ.setdefault("JWT_SECRET", "this-is-a-really-long-secret-to-avoid-warnings-caused-by-the-jwt-library")
os.environ.setdefault("DISCORD_BOT_TOKEN", "dummy")
os.environ.setdefault("DISCORD_MODERATOR_ROLE_ID", "1234")
os.environ.setdefault("SMTP_HOST", "dummy")
os.environ.setdefault("SMTP_PORT", "1234")
os.environ.setdefault("SMTP_USERNAME", "dummy")
os.environ.setdefault("SMTP_PASSWORD", "dummy")
os.environ.setdefault("SMTP_CRYPTO", "SSL")
os.environ.setdefault("FROM_EMAIL_ADDRESS", "dummy")
